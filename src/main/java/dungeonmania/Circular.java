...
public class Circular implements Movement {
    ...
    public Position getNextPosition(List<Entities> allEntities, Position currentPosition) {
        ...
        List<Entities> listBoulders = allEntities.stream()
                .filter((Entities e) -> e.getClass().toString().equals("boulder")).collect(Collectors.toList());
        if (listBoulders.stream()
                .anyMatch((Entities p) -> p.getPosition().equals(movementTrajectory.get(nextRotation)))) {
            if (isReversed) {
